...
async function performFirstTimeSetup(db) {
  ...
  await db.exec("CREATE TABLE IF NOT EXISTS mailboxes (id INTEGER PRIMARY KEY AUTOINCREMENT, address TEXT NOT NULL UNIQUE, local_part TEXT NOT NULL, domain TEXT NOT NULL, password_hash TEXT, created_at TEXT DEFAULT CURRENT_TIMESTAMP, last_accessed_at TEXT, expires_at TEXT, is_pinned INTEGER DEFAULT 0, can_login INTEGER DEFAULT 0, forward_to TEXT DEFAULT NULL, is_favorite INTEGER DEFAULT 0, totp_secret TEXT, totp_enabled INTEGER DEFAULT 0, totp_backup_codes TEXT);");
  ...
  await db.exec("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT NOT NULL UNIQUE, password_hash TEXT, role TEXT NOT NULL DEFAULT 'user', can_send INTEGER NOT NULL DEFAULT 0, mailbox_limit INTEGER NOT NULL DEFAULT 10, created_at TEXT DEFAULT CURRENT_TIMESTAMP, totp_secret TEXT, totp_enabled INTEGER DEFAULT 0, totp_backup_codes TEXT);");
  ...
  // 部分初始化的库：已存在的旧表可能缺新列，建表路径也无条件补迁移
  await migrateMailboxesFields(db);
  await migrateUsersFields(db);
  await migrateSentEmailsFields(db);
}
...
async function migrateMailboxesFields(db) {
  try {
    ...
    // 2FA：TOTP 密钥 / 开关 / 备份码（向后兼容，纯加列）
    if (!columnNames.includes('totp_secret'))       await db.exec("ALTER TABLE mailboxes ADD COLUMN totp_secret TEXT;");
    if (!columnNames.includes('totp_enabled'))      await db.exec("ALTER TABLE mailboxes ADD COLUMN totp_enabled INTEGER DEFAULT 0;");
    if (!columnNames.includes('totp_backup_codes')) await db.exec("ALTER TABLE mailboxes ADD COLUMN totp_backup_codes TEXT;");
  } catch (error) {
...
/**
 * 迁移 users 表字段（向后兼容）：2FA 列 totp_secret / totp_enabled / totp_backup_codes
 * @param {object} db - 数据库连接对象
 * @returns {Promise<void>}
 */
async function migrateUsersFields(db) {
  try {
    const columns = await db.prepare("PRAGMA table_info(users)").all();
    const columnNames = (columns.results || []).map(c => c.name);
    if (!columnNames.includes('totp_secret'))       await db.exec("ALTER TABLE users ADD COLUMN totp_secret TEXT;");
    if (!columnNames.includes('totp_enabled'))      await db.exec("ALTER TABLE users ADD COLUMN totp_enabled INTEGER DEFAULT 0;");
    if (!columnNames.includes('totp_backup_codes')) await db.exec("ALTER TABLE users ADD COLUMN totp_backup_codes TEXT;");
  } catch (error) {
    console.error('users 字段迁移失败:', error);
    // 不抛出异常，允许继续运行
  }
}
